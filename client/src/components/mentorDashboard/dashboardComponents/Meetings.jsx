                ...
                <div className='flex flex-wrap gap-3 justify-center items-center my-3'>
                    {meetings && meetings?.map(item =>
                        <div key={item?._id} className='p-2 relative min-w-[260px] border-2 shadow-lg rounded-xl px-4 m-2'>
                            ...
                            <button onClick={()=>handleJoinMeeting(item.roomId)}
                                className={`bg-indigo-500 cursor-pointer hover:bg-indigo-700 px-4 py-2 mx-auto inline-block my-2 text-xs font-bold text-white rounded-lg 
                                    ${(item?.isExpired) ? "cursor-not-allowed" : ''}`} disabled={item?.isExpired}>
                                Join</button>
                            {
            ...
            <div className='flex flex-col justify-center items-center my-2 py-2 '>
                {
                    mentees?.map((item) =>
                        <div key={item?._id} onClick={() => {
                            ...
                            className={`p-1 my-2  inline-flex cursor-pointer
